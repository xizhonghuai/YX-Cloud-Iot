...
public class TcpServer extends AbstractBootServer {
    ...
    public boolean init() {
        ...
        try {
            ...
            // 绑定逻辑处理器
            Class<?> clazz = ToolUtils.getClass(par.getHandlerJarFile(), par.getHandlerClassName());
            this.businessHandler = (BusinessHandler) clazz.newInstance();
            ioAcceptor.setHandler(new Handler(this.businessHandler));

            //测试本地
//
//            BusinessHandler myHandler = new MyHandler();
//
//            ioAcceptor.setHandler(new Handler(myHandler));
